...
if processorType == "cpu":
    ...
    verbose=False # deactivate verbose in case of cluster setup
...
import datadefinitions.bpi2018 as datadef
datadef = datadef.BPI2018()

if len(sys.argv) > 1:
    param = float(sys.argv[1])
utility.run.Train_And_Evaluate(
    #data
    eventlog="datasets/bpi2018.csv",  # file to read in
    ...
    max_sequencelength=200,       # maximum allowed sequence length
    #framework/ann specifics
    batch_size=256,                   # batch size (set to 1 for stateful)
    ...
    #debug
    save_model=True,                # saves the model file each checkpoint (highly recommended to set to False for TPU usage)
