...
import datadefinitions.bpi2017 as datadef
datadef = datadef.BPI2017()
...
for i in range(100):
    utility.run.Train_And_Evaluate(
        ...
        datageneration_pattern = DataGenerationPattern.Generator, # Fit: uses the classical approach and loads everything into memory; Generator: uses the python generator pattern
        #regularization/dataset manipulation
        bagging=True,                   # perform bagging? 
        bagging_putback=True,            # (if bagging) elements get put back and can be drawn again 
        bagging_size=0.6,                # (if bagging) the split to bag train data
        ...
        bidirectional=False,              # defines if the recurrent layers are bidirectional
        ...
        processor=Processor.TPU,         # processor, cpu, gpu or tpu (gpu uses CUDNN based algorithms for lstm and gru if cudnn is set to true)
        cudnn=True,                      # (if GPU) utilizes special nVidia-CuDNN LSTM and GRU implementations
        #debug
        save_model=False,                # saves the model file each checkpoint (set to False for TPU usage)
        tensorboard=False,               # outputs tensorboard compatible eventlog into ./graph folder for visualization
        target_host='http://dumpster.chemsorly.com:80/',                  # sends a POST request with all the result files (result file and model) to the host
