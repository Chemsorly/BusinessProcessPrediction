    ...
    def CreateMatrices(self,sentences,args):
        ...
        for i in range (len(sentences[0])):
            ...
            for j in range (len(sentences[0][i])):   
                ...
                X[i, j+leftpad, catvectorlen+1] = ((sentences[1][i][j]/divisors[1]) * weights[1])  #time since last event
                X[i, j+leftpad, catvectorlen+2] = ((sentences[2][i][j]/divisors[2]) * weights[2])  #timestamp
                X[i, j+leftpad, catvectorlen+3] = ((sentences[3][i][j]/divisors[3]) * weights[3])  #duration
                X[i, j+leftpad, catvectorlen+4] = ((sentences[4][i][j]/divisors[4]) * weights[4])  #planned duration
                X[i, j+leftpad, catvectorlen+5] = ((sentences[5][i][j]/divisors[5]) * weights[5])  #planned timestamp

            y_t[i,0] = sentences[6][i][0]/divisors[6] #already offsetted
